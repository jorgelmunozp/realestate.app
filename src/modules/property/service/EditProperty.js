...
export const EditProperty = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const cleanedImage = itemPropertyImage?.file && itemPropertyImage.file.trim() !== '' ? mapPropertyImageToDto(itemPropertyImage) : null;

      const cleanedOwner = itemOwner?.name?.trim() && itemOwner?.address?.trim() && (itemOwner?.photo?.trim() || itemOwner?.ownerPhotoPreview?.trim()) && itemOwner?.birthday?.trim() ? mapOwnerToDto(itemOwner) : null;

      const cleanedTraces = Array.isArray(itemPropertyTrace) && itemPropertyTrace.length > 0 ? itemPropertyTrace.filter((t) => t.name?.trim() && t.dateSale?.trim() && Number(t.value) > 0).map(mapPropertyTraceToDto) : [];
  ...
  if (!itemProperty) return <p style={{ textAlign: 'center', color: 'red' }}>Inmueble no encontrado</p>;
  ...
  return (
    <div className="editproperty-container">
      <form className="editproperty-card" onSubmit={handleSubmit}>
        ...
        <Button id="updateButton" type="submit" variant="contained" color={updating ? 'secondary' : 'primary'} disabled={updating} aria-label="update button">
          {updating ? 'Actualizando...' : 'Actualizar Propiedad'}
        </Button>
        <Button id="cancelButton" variant="outlined" color="secondary" onClick={() => goBackToHome(false)} aria-label="cancel button">
          Cancelar
        </Button>
      </form>
    </div>
